...
async function run(): Promise<void> {
  try {
    ...
    const globber = await glob.create('**/terrakube.json')
    ...
    for await (const file of globber.globGenerator()) {
      ...
      const isFolderChanged = githubActionInput.terrakubeFolder
        .split(' ')
        .includes(workspaceFolder)
      core.info(`Folder ${workspaceFolder} was_changed: ${isFolderChanged}`)
      const workspaceName =
        terrakubeData.workspace && terrakubeData.workspace.trim() !== ''
          ? terrakubeData.workspace
          : workspaceFolder
      ...
      if (isFolderChanged) {
        ...
        core.info(
          `Running Workspace ${workspaceName} with Template ${githubActionInput.terrakubeTemplate}`
        )
        core.info(`Checking if workspace ${workspaceName}`)

        const organizationId = await terrakubeClient.getOrganizationId(
          githubActionInput.terrakubeOrganization
        )
        if (!organizationId) {
          core.error(
            `Organization not found: ${githubActionInput.terrakubeOrganization} in Endpoint: ${githubActionInput.terrakubeEndpoint}`
          )
          core.endGroup()
          continue
        }

        let workspaceId = await terrakubeClient.getWorkspaceId(
          organizationId,
          workspaceName
        )
        if (!workspaceId) {
          ...
          if (githubActionInput.terrakubeSshKeyName) {
            ...
            sshId = await terrakubeClient.getSshId(
              organizationId,
              githubActionInput.terrakubeSshKeyName
            )
            core.info(`Ssh Id: ${sshId}`)
          }
          workspaceId = await terrakubeClient.createWorkspace(
            organizationId,
            workspaceName,
            terrakubeData.terraform,
            `/${workspaceName}`,
            githubActionInput.terrakubeRepository,
            githubActionInput.branch,
            sshId
          )
        }

        core.info(`Searching template ${githubActionInput.terrakubeTemplate}`)
        const templateId = await terrakubeClient.getTemplateId(
          organizationId,
          githubActionInput.terrakubeTemplate
        )
        if (!templateId) {
          core.error(
            `Template not found: ${githubActionInput.terrakubeTemplate} in Organization: ${githubActionInput.terrakubeOrganization}`
          )
          core.endGroup()
          continue
        }
        ...
        const jobId = await terrakubeClient.createJobId(
          organizationId,
          workspaceId,
          templateId
        )
        ...
        await checkTerrakubeLogs(
          terrakubeClient,
          githubActionInput.githubToken,
          organizationId,
          jobId,
          workspaceName,
          githubActionInput.showOutput
        )
...
async function checkTerrakubeLogs(
  ...
  while (
    jobResponseJson.data.attributes.status !== 'completed' &&
    jobResponseJson.data.attributes.status !== 'failed'
  ) {
    await sleep(5000)
  ...
  for (let index = 0; index < Object.keys(jobSteps).length; index++) {
    ...
    const response: httpm.HttpClientResponse = await httpClient.get(
      `${jobSteps[index].attributes.output}`,
  ...
  if (show_output) {
    core.info('Setup Octokit client')
